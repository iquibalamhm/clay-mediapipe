 ...
 */
void DrawSprites::draw_text(std::string text, glm::vec2 const &anchor, float scale, glm::u8vec4 const &tint, glm::vec2 *anchor_out) {
...
//    moving_anchor.y -= font_height;
	for (size_t pos = 0; pos < text.size(); pos++){
	    ...
	    if (anchor_out) {
            ...
            anchor_out->x = std::max(anchor_out->x, moving_anchor.x);
            anchor_out->y = std::min(anchor_out->y, moving_anchor.y);
	    } else {
